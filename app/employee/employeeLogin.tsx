...
export default function EmployeeLogin() {
  ...
  const handleChange = async (text) => {
    ...
    const response = await apiFunction(otpApi, null, { phone: phoneNumber, role: "employee", otp: text }, "post", false);
    if (response) {
      ...
      if (response?.user?.profile) {
        ...
        router.push("/employee/createProfile");
      }
  ...
  const handlePhone = async (value) => {
    ...
    if (value.length === 10) {
      const response = await apiFunction(mobileApi, null, { phone: value, role: "employee" }, "post", false)
