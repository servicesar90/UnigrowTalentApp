...
const CompanyProfileImage = ({ open, label, onClose, metaData }) => {
...
const handleUploadSuccess = () => {
  ...
  dispatch(fetchEmployerProfile()).then(() => {
    ...
    <Modal visible={open} transparent animationType="slide" onRequestClose={onClose}>
      <View className="flex-1 bg-black/50 justify-center items-center">
        <View className="bg-white p-6 rounded-2xl w-80 items-center">
          ...
          {preview && label !== "Upload Image" ? (
            <Image source={{ uri: preview }} className="w-32 h-32 rounded-full mb-4" />
          ...
          >
            <Text className="text-white text-center font-medium">Choose File</Text>
          </TouchableOpacity>
